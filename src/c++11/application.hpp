...
namespace application {
    ...
    inline ApplicationArguments parse_arguments(int argc, char *argv[])
    {
        ...
        while (arg_processing < argc) {
            ...
            || strcmp(argv[arg_processing], "--qos_file") == 0)) {
                qos_file = argv[arg_processing + 1];
                arg_processing += 2;
            } else if ((argc > arg_processing + 1)
        ...
        if (qos_file.empty()) {
            std::cout << "Missing -q or --qos_file argument." << std::endl;
            show_usage = true;
            parse_result = ParseReturn::failure;
        }
        ...
        return ApplicationArguments(parse_result, domain_id, qos_file, sample_count, verbosity);
    }
